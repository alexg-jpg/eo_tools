...
#Obtain effective index profile for our waveguide
    ...
    return [neff,h_m,q_m,p_m, beta[zeros],k0]
...
def get_field_profile(d, w_rib, h_slab, wavelength, n1, n2): 
    ...
    eta = np.sqrt(mu/(epsilon_o*100))# The "characteristic impedance of the medium"
    pts = 500
    ...
    modeResultsTM = get_neff_tm(w_rib,1.55e-6,modeResultsTEEdge[0],modeResultsTE[0][0],modeResultsTEEdge[0]) 
    '''
    ...
    qTM = modeResultsTM[2]
    pTM = modeResultsTM[3]
    qBarTM=(modeResultsTE[0]**2)/(modeResultsTEEdge[0]**2)*qTM #Syntax taken from Matlab script by Lukas Chrostowski, 2012
    pBarTM=(modeResultsTE[0]**2)/(modeResultsTEEdge[0]**2)*pTM
    betaTM = modeResultsTM[4]

    
    neffCentre = modeResultsTE[0]
    neffEdge = modeResultsTEEdge[0]
    ...
    tEff = (qBarTM**2+hTM**2)/qBarTM**2 * (w_rib/neffCentre**2 + (qTM**2+hTM**2)/(qBarTM**2+hTM**2)/neffEdge**2/qTM + ( pTM**2+hTM**2)/(pTM**2+hTM**2)/neffEdge**2/pTM)
    cTM = 2 * np.sqrt(omega0*epsilon_o*100 / (np.abs(betaTM)*tEff))
    #eTE = np.zeros(pts*3)
    hFieldTM = np.array([np.zeros(pts*3),np.zeros(pts*3)])
    ...
    for i in range(0,len(cTM)):
        ...
        #region 3 E field. We're assuming that the refractive indexes of the BOX and cladding are identical.
        hTM3 = cTM[i]*(hTM[i]/qBarTM[i]*np.cos(hTM[i]*w_rib)+np.sin(hTM[i]*w_rib))*np.exp(-pTM[i]*(y3-w_rib/2))

        hFieldTM[i] += np.concatenate((hTM1,hTM2,hTM3),axis=None)
    
    ny=np.concatenate((modeResultsTEEdge[0]*np.ones(pts), modeResultsTE[0][0]*np.ones(pts), modeResultsTEEdge[0]*np.ones(pts)),axis=None)
    
    eFieldTM = hFieldTM[0] / ny * eta
...
def get_neff_v(n_a,n_d,w_pn, w_rib, x_offset,d,h_slab, wavelength, n1, n2, v_a):
    ...
    w_d0 = get_depletion_width(n_a,n_d,V_bi,0)

    w_dp0 = w_d0[1]
    w_dn0 = w_d0[2]
    w_d0 = w_d0[0]
    #Obtain 1D transverse carrier profile of our WG.
    [n_profile0, p_profile0] = get_carrier_concentrations(n_a,n_d,0, w_pn, w_rib,w_d0, w_dn0, w_dp0, x_offset) 

    #n_profile0 *= 10**6 #Unit conversion from cm^-3 to m^-3
    #p_profile0 *= 10**6
    ...
    plt.plot(y,n_profile)
    plt.plot(y,n_profile0)
    plt.show()
...
get_neff_v(1e17,1e17,1.2e-6, 5e-7,x_offset,220e-9,h_slab, 1.55e-6, n1, n2, v_a)
